...
export default function ShareButtons({ locale = 'pl' }: ShareButtonsProps) {
    const title = locale === 'pl' ? 'Udostępnij artykuł' : 'Share article';
    ...
    return (
        <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6 backdrop-blur-sm shadow-lg">
            <h3 className="text-xs font-semibold tracking-wider uppercase text-slate-500 mb-4 pb-2 border-b border-slate-700/50">
                {title}
            </h3>
            <div className="flex flex-col gap-2">
                ...
                >
                    ...
                    <span>{locale === 'pl' ? 'Kopiuj link' : 'Copy link'}</span>
                </button>
            </div>
        </div>
    );
